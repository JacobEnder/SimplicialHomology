...
function detect_3_cycles(adj)
    ...
    # For triangle counting: trace(A³) / 6 gives number of triangles
    # But we want to list them, so we use the neighbor intersection approach
    triangles = []
...
function detect_4_cycles(adj)
    ...
    # Build all wedges (paths of length 2) efficiently
    wedges = []
    for v in vertices
